 ...
 */
export function buildOrbitFilter(q) {
    const now = Date.now();
    return (doc) => {
        ...
        if (doc.ttl !== undefined && doc.ttl < now)
            return false;
        if (q.type !== undefined && doc.type !== q.type)
            return false;
        if (q.namespace !== undefined && doc.namespace !== q.namespace)
            return false;
        if (q.project !== undefined && doc.source.project !== q.project)
            return false;
        // PeerId namespace filter — matches publishing agent
        if (q.peerId !== undefined && doc.source.peerId !== q.peerId)
            return false;
        // Collection filter
        if (q.collection !== undefined && doc.collection !== q.collection)
            return false;
        // Content format filter
        if (q.contentFormat !== undefined && doc.contentEnvelope?.format !== q.contentFormat)
            return false;
        if (q.minConfidence !== undefined && doc.confidence < q.minConfidence)
            return false;
        if (q.since !== undefined && doc.source.timestamp < q.since)
            return false;
        if (q.until !== undefined && doc.source.timestamp > q.until)
            return false;
 ...
 */
export function resolveHeads(chunks) {
    if (chunks.length === 0)
        return [];
    ...
    const supersedesGroups = new Map();
    ...
    for (const group of supersedesGroups.values()) {
        if (group.length === 1) {
            ...
            const winner = group.reduce((best, c) => c.source.timestamp > best.source.timestamp ? c : best);
            result.push(winner);
        }
    }
    return result;
 ...
 * 4. Apply limit
 */
export function applyQuery(chunks, q) {
