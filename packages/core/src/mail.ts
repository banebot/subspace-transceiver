...
export type MailMessage =
  | { type: 'deposit'; envelope: MailEnvelope }
  | { type: 'check'; recipientPeerId: string; since?: number; limit?: number }
  | { type: 'check-response'; envelopes: MailEnvelope[]; hasMore: boolean }
  | { type: 'ack'; envelopeIds: string[] }
  | { type: 'ack-response'; purged: number }
  | { type: 'deposit-response'; ok: boolean; error?: string }
 ...
 */
export async function encryptMailPayload(
  payload: MailPayload,
  senderKey: PrivateKey,
  recipientPeerId: string,
  envelopeId: string,
): Promise<{ payload: string; ephemeralPubKey: string; nonce: string; authTag: string }> {
  ...
  if (!senderPubKeyBytes) throw new Error('Cannot extract sender public key bytes')
  ...
  if (!recipientPubKeyBytes) throw new Error('Cannot extract recipient public key bytes from PeerId')
 ...
 */
export async function decryptMailEnvelope(
  envelope: MailEnvelope,
  recipientKey: PrivateKey,
  senderPeerId: string,
): Promise<MailPayload> {
  const recipientPubKeyBytes = recipientKey.publicKey.raw
  if (!recipientPubKeyBytes) throw new Error('Cannot extract recipient public key bytes')

  const senderPeer = peerIdFromString(senderPeerId)
  const senderPubKeyBytes = senderPeer.publicKey?.raw
  if (!senderPubKeyBytes) throw new Error('Cannot extract sender public key bytes from PeerId')
  ...
  try {
    ...
    return JSON.parse(plaintext.toString('utf8')) as MailPayload
  } catch {
...
export async function signEnvelope(
  envelope: Omit<MailEnvelope, 'signature'>,
  senderKey: PrivateKey,
): Promise<MailEnvelope> {
  const signingData = Buffer.from(
    `${envelope.id}|${envelope.from}|${envelope.to}|${envelope.payload}|${envelope.ephemeralPubKey}|${envelope.nonce}|${envelope.timestamp}`,
    'utf8'
  )
 ...
 */
export async function verifyEnvelopeSignature(envelope: MailEnvelope): Promise<boolean> {
  try {
 ...
 */
export function isEnvelopeExpired(envelope: MailEnvelope): boolean {
...
export function createEnvelope(
  opts: {
    ...
    to: string
    envelopeId: string
    encrypted: { payload: string; ephemeralPubKey: string; nonce: string; authTag: string }
    ttl?: number
    contentType?: string
  }
