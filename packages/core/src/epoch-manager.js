...
export const DEFAULT_EPOCH_CONFIG = {
 ...
 */
export function computeEpochId(nowMs, epochDurationMs) {
 ...
 */
export function epochStartMs(epochId, epochDurationMs) {
...
export class EpochManager extends EventEmitter {
    ...
    constructor(orbitdb, networkKeys, namespace, config, dataDir, envelopeKey, currentEpochId, epochs) {
        super();
     ...
     */
    static async create(orbitdb, networkKeys, namespace, config = DEFAULT_EPOCH_CONFIG, dataDir, envelopeKey = networkKeys.envelopeKey) {
        ...
        const epochEntries = await Promise.all(epochIds.map(async (epochId) => {
            const store = await EpochManager._openEpochStore(orbitdb, networkKeys, namespace, epochId, envelopeKey);
            return [epochId, store];
        }));
        const epochs = new Map(epochEntries);
        const manager = new EpochManager(orbitdb, networkKeys, namespace, config, dataDir, envelopeKey, currentEpochId, epochs);
    ...
    static async _openEpochStore(orbitdb, networkKeys, namespace, epochId, envelopeKey) {
    ...
    async query(q) {
        ...
        for (const epochId of this._orderedEpochIds()) {
            const store = this.epochs.get(epochId);
            const chunks = await store.query(q).catch(() => []);
            results.push(...chunks);
        }
     ...
     */
    async maybeRotateEpoch(nowMs = Date.now()) {
        const expectedEpochId = computeEpochId(nowMs, this.config.epochDurationMs);
        if (expectedEpochId === this.currentEpochId)
            return false;
     ...
     */
    async rotateEpoch(newEpochId, nowMs = Date.now()) {
        this.migrationInProgress = true;
        try {
            // Open new epoch store
            const newStore = await EpochManager._openEpochStore(this.orbitdb, this.networkKeys, this.namespace, newEpochId, this.envelopeKey);
     ...
     */
    async _migrateChunks(fromEpochId, toStore, nowMs) {
        const fromStore = this.epochs.get(fromEpochId);
        if (!fromStore)
            return { migrated: 0, abandoned: 0 };
        let migrated = 0;
        let abandoned = 0;
        try {
            ...
            for (const chunk of chunks) {
                ...
                await toStore.put(chunk).catch((err) => {
        ...
        console.log(`[subspace] EpochManager: migrated ${migrated}, abandoned ${abandoned} chunks from epoch ${fromEpochId} (${this.namespace})`);
     ...
     */
    async dropExpiredEpochs(nowMs = Date.now()) {
        const retentionCutoffMs = nowMs - (this.config.retainEpochs + 1) * this.config.epochDurationMs;
        const dropped = [];
    ...
    async _measureAndDeleteEpochDir(epochId) {
     ...
     */
    async getEpochInfo(nowMs = Date.now()) {
        const infos = [];
        for (const epochId of this._orderedEpochIds()) {
            const store = this.epochs.get(epochId);
            const chunks = await store.list().catch(() => []);
    ...
    currentStore() {
        ...
        if (!store)
            throw new Error(`EpochManager: current epoch store missing (${this.currentEpochId})`);
        return store;
    }
    /** Returns epoch IDs sorted newest-first (for reads: prefer newest data). */
    _orderedEpochIds() {
        return [...this.epochs.keys()].sort((a, b) => epochStartMs(b, this.config.epochDurationMs) - epochStartMs(a, this.config.epochDurationMs));
    }
...
//# sourceMappingURL=epoch-manager.js.map