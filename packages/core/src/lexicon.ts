 ...
 */
export function validateLexiconSchema(schema: unknown): ValidationResult {
  const errors: string[] = []
  if (typeof schema !== 'object' || schema === null) {
    return { valid: false, errors: ['Schema must be an object'] }
  }
  const s = schema as LexiconSchema
  if (s.lexicon !== 1) errors.push('lexicon must be 1')
  if (typeof s.id !== 'string' || !isValidNSID(s.id)) errors.push(`id must be a valid NSID, got: "${s.id}"`)
  if (typeof s.revision !== 'number' || s.revision < 1 || !Number.isInteger(s.revision)) {
    errors.push('revision must be a positive integer')
  }
  if (typeof s.defs !== 'object' || s.defs === null) errors.push('defs must be an object')
  else if (!s.defs.main) errors.push('defs.main is required')
...
export function validateRecordData(
  ...
  for (const [key, fieldSchema] of Object.entries(record.properties)) {
    if (!(key in data)) continue  // optional field absent — OK
...
function validateField(value: unknown, schema: FieldSchema, path: string): string[] {
  const errors: string[] = []

  if (schema.type === 'unknown') return errors  // any value accepted
    ...
    case 'object': {
      ...
      if (schema.properties) {
        for (const [k, fs] of Object.entries(schema.properties)) {
          if (k in obj) errors.push(...validateField(obj[k], fs, `${path}.${k}`))
        }
      }
      break
    }
...
const MEMORY_RECORD_BASE_PROPERTIES: Record<string, FieldSchema> = {
 ...
 */
export const BUILT_IN_SCHEMAS: LexiconSchema[] = [
  {
    lexicon: 1,
