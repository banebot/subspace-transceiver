 ...
 */
const SubspaceAccessController = (options = {}) => async ({ name, address }) => {
    const { maxContentBytes = 65_536, maxEnvelopeBodyBytes = 262_144, requireSignatures = false, } = options;
    ...
    const canAppend = async (entry) => {
        try {
            ...
            const contentField = (doc._encrypted === true)
                ...
                : (doc.content ?? '');
            if (Buffer.byteLength(contentField, 'utf8') > maxContentBytes)
                return false;
            // 4. Envelope body size limit
            const envelopeBodyField = (doc._encrypted === true)
                ? (doc.encryptedEnvelopeBody ?? '')
                : (doc.contentEnvelope?.body ?? '');
            if (Buffer.byteLength(envelopeBodyField, 'utf8') > maxEnvelopeBodyBytes)
                return false;
            ...
            const peerId = doc.source?.peerId;
            ...
            if (!isEncrypted && signature && peerId) {
                try {
                    ...
                    if (pubKey) {
                        ...
                        const valid = await verifyChunkSignature(doc, pubKey);
                        if (!valid)
                            return false;
                    }
                }
