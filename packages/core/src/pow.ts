...
function windowIndex(windowMs: number): number {
...
export function currentChallenge(
  peerId: string,
  scope: string,
  windowMs: number = DEFAULT_POW_WINDOW_MS,
  windowIdx?: number,
): string {
 ...
 * - At 16 bits: ~2ms.
 */
export async function mineStamp(
  peerId: string,
  scope: string,
  bits: number,
  windowMs: number = DEFAULT_POW_WINDOW_MS,
): Promise<HashcashStamp> {
...
export function verifyStamp(
  ...
  if (stamp.bits < bits) return false
  ...
  if (expectedHash !== stamp.hash) return false

  // 3. Hash must actually have the required leading zero bits.
  if (!hasLeadingZeroBits(stamp.hash, bits)) return false
...
export class StampCache {
  private cache = new Map<string, StampCacheEntry>()

  private cacheKey(scope: string, bits: number, windowMs: number): string {
    return `${scope}:${bits}:${windowMs}`
  }

  private currentWindowIndex(windowMs: number): number {
    return Math.floor(Date.now() / windowMs)
  }
  ...
  get(scope: string, bits: number, windowMs: number): StampCacheEntry | null {
    ...
    if (entry.windowIndex !== this.currentWindowIndex(windowMs)) return null
    return entry
  }
