...
function encodeMsg(msg: unknown): Uint8Array {
  return encoder.encode(JSON.stringify(msg))
}

function decodeMsg<T>(bytes: Uint8Array): T {
  return JSON.parse(decoder.decode(bytes)) as T
}

// libp2p v3 stream type
interface Lp2pStream extends AsyncIterable<Uint8Array | { subarray(): Uint8Array }> {
  send(data: Uint8Array): boolean
  close(opts?: { signal?: AbortSignal }): Promise<void>
  abort(err: Error): void
}

async function writeMsg(stream: Lp2pStream, msg: unknown): Promise<void> {
...
async function readMsg<T>(stream: Lp2pStream): Promise<T | null> {
  for await (const chunk of lp.decode(stream as AsyncIterable<Uint8Array>)) {
    ...
    return decodeMsg<T>(bytes)
  }
  return null
}

// ---------------------------------------------------------------------------
// Server side — register the mailbox protocol handler on a libp2p node
// ---------------------------------------------------------------------------

export interface MailboxHandlerOptions {
  relayStore: IRelayStore
  inboxStore: IInboxStore
  recipientPeerId: string          // This node's PeerId (for filtering "my" mail)
  recipientKey: PrivateKey         // For decrypting mail addressed to this node
...
export async function registerMailboxProtocol(
  ...
  await node.handle(MAILBOX_PROTOCOL, async (stream: unknown, _connection: unknown) => {
    const s = stream as unknown as Lp2pStream
    try {
      const msg = await readMsg<MailMessage>(s)
      if (!msg) return
...
export async function sendMail(
  ...
  try {
    ...
    }) as unknown as Lp2pStream

    await writeMsg(stream, depositMsg)
    const response = await readMsg<MailMessage>(stream)
  ...
  for (const relayPeer of relayPeers) {
    try {
      ...
      for (const envelope of response.envelopes) {
        if (envelope.to !== opts.recipientPeerId) continue
        if (isEnvelopeExpired(envelope)) { ackIds.push(envelope.id); continue }
