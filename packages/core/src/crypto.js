 ...
 */
export function validatePSK(psk) {
    if (psk.length < 16) {
        throw new CryptoError(`PSK too short: ${psk.length} chars. Minimum is 16. ` +
            'Generate a secure PSK with: openssl rand -hex 32', ErrorCode.PSK_TOO_SHORT);
    }
    if (psk.length < 32) {
        console.warn(`[subspace] WARNING: PSK is only ${psk.length} characters. ` +
 ...
 */
export function deriveNetworkKeys(psk) {
    const keyMaterial = Buffer.from(psk, 'utf8');
    // Zero salt — PSK is the entropy source. See module JSDoc for rationale.
    const salt = Buffer.alloc(32);
    const derive = (info, len) => Buffer.from(hkdfSync('sha256', keyMaterial, salt, Buffer.from(info, 'utf8'), len));
    return {
 ...
 */
export function encryptEnvelope(plaintext, key) {
 ...
 */
export function decryptEnvelope(ciphertext, iv, tag, key) {
    try {
        ...
        throw new CryptoError('Decryption failed — data may be tampered or the key is incorrect.', ErrorCode.DECRYPT_FAILED, err);
    }
}
