 ...
 */
export async function createLibp2pNode(agentPrivateKey, options = {}) {
    const { port = 0, minConnections = 5, maxConnections = 50, trustedBootstrapPeers = [], relayAddresses, connectionPruner: prunerOpts = {}, } = options;
    ...
    const autoDial = (id) => {
        if (id.toString() === node.peerId.toString())
            return;
        if (node.getPeers().some(p => p.toString() === id.toString()))
            return;
    ...
    const mdnsService = node.services['mdns'];
    if (mdnsService?.addEventListener != null) {
        mdnsService.addEventListener('peer', (evt) => {
            const peerInfo = evt.detail;
            if (!peerInfo?.id || peerInfo.id.toString() === node.peerId.toString())
                return;
            if (node.getPeers().some(p => p.toString() === peerInfo.id.toString()))
                return;
 ...
 */
export function derivePeerId(agentPrivateKey) {
