...
export class BacklinkIndex {
    ...
    removeChunk(chunk) {
        ...
        if (chunk.supersedes) {
            ...
            if (set) {
                set.delete(chunk.id);
                if (set.size === 0)
                    this.index.delete(chunk.supersedes);
            }
        }
    }
    /**
     ...
     */
    static getLinks(chunk) {
        const links = [...(chunk.links ?? [])];
        // Surface the supersedes field as a synthetic link entry
        if (chunk.supersedes) {
            const alreadyIndexed = links.some(l => l.target === chunk.supersedes && l.rel === 'supersedes');
            if (!alreadyIndexed) {
...
//# sourceMappingURL=backlink-index.js.map