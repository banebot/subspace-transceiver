...
export function isValidNSID(nsid: string): boolean {
  if (!nsid || nsid.length > MAX_NSID_LEN) return false
 ...
 */
export function parseNSID(nsid: string): ParsedNSID {
  if (!isValidNSID(nsid)) {
 ...
 */
export function nsidMatches(nsid: string, pattern: string): boolean {
 ...
 */
export function memoryTypeToNSID(type: string): string | null {
  return (BUILT_IN_NSIDS as Record<string, string>)[type] ?? null
}
 ...
 */
export function nsidToMemoryType(nsid: string): string | null {
  for (const [type, n] of Object.entries(BUILT_IN_NSIDS)) {
    if (n === nsid) return type
  }
  return null
}
